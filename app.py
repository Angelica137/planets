...
def add_planet():
    ...
    if test:
        ...
        new_planet = Planet(planet_name=planet_name, planet_type=planet_type, home_star=home_star, mass=mass, distance=distance)
...
def update_planet():
    ...
    if planet:
        ...
        distance = float(request.form['distance'])
        db.session.commit()
        return jsonify(message="you have updated the planet " + planet.planet_name), 202
    else:
        return jsonify(message="that planet does not exist, do you want to create it?"), 404
...
def remove_planet(planet_id: int):
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message='Plane deleted'), 202
    else:
        return jsonify(message="That planed does not exist"), 404
